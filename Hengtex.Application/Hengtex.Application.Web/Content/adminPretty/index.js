...
(function ($) {
    ...
    $.hengtextab = {
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.hengtextab.calSumWidth($(".lea-tabs").children().not(".menuTabs"));
        ...
        init: function () {
            $('.menuTabs').on('click', '.menuTab .tab_close', $.hengtextab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.hengtextab.activeTab);

            $('.tabLeft').on('click', $.hengtextab.scrollTabLeft);
            $('.tabRight').on('click', $.hengtextab.scrollTabRight);
            $('.tabReload').on('click', $.hengtextab.refreshTab);
    ...
    $.hengtexindex = {
        ...
        loadMenu: function (isInit) {
            var flag = false;
            var topMenuWidth = $('.lea-Head').width() - $.hengtextab.calSumWidth($(".lea-Head").children().not(".left-bar"));
            ...
            $.each(data, function (i) {
                ...
                if (row.ParentId == "0") {
                    ...
                    _itemHtml += '</a>';
                    var childNodes = $.hengtexindex.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
                    if (childNodes.length > 0) {
                        _itemHtml += '<div class="popover-menu"><div class="arrow"><em></em><span></span></div><ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.hengtexindex.jsonWhere(data, function (v) { return v.ParentId == subrow.ModuleId });
                            _itemHtml += '<li>';
            ...
            if (isInit || flag) {
                ...
                $('.menuiframe').unbind();
                $('.menuiframe').on('click', $.hengtextab.addTab);
