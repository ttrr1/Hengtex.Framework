...
namespace Hengtex.Application.Web.Areas.AppManage.Controllers
{
    ...
    /// 描 述：系统按钮
    /// </summary>
    public class AppModuleButtonController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeListJson(string moduleId)
        {
            ...
            if (data != null)
            {
                ...
                foreach (AppModuleButtonEntity item in data)
                {
        ...
        {
            var data = from items in moduleButtonJson.ToList<AppModuleButtonEntity>() orderby items.SortCode select items;
