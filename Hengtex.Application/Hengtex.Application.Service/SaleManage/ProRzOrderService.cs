...
namespace Hengtex.Application.Service.SaleManage
{
    ...
    /// 描 述：发货管理
    /// </summary>
    public class ProRzOrderService : RepositoryFactory<ProRzOrderEntity>, IProRzOrderService
    {
        ...
        {
            var expression = LinqExtensions.True<ProRzOrderEntity>();
        ...
        public ProRzOrderEntity GetEntity(string keyValue)
        {
          ...
          return  this.ERPRepository().FindList(strSql.ToString(), parameter).FirstOrDefault<ProRzOrderEntity>();
        ...
        {
            string sql = "select ppg_batch,SUM(ISNULL(ppg_lengthReal,0)) as countAll from mft_pack_packages d left join mft_pack_packs m on d.ppg_pack=m.mpp_num where FlagDelete=0 and mpp_packType<>'重打卷'  AND (ppg_batch in (select batch  from temp_batches where userName='" + user + "') ) group by ppg_batch";
            return this.ERPRepository().FindTable(sql);
        }
