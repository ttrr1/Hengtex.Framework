 ...
 app .factory('$hengtexTopAlert', [
  ...
  .factory('$hengtexHttp', [
    ...
    '$hengtexTopAlert',
    'UserInfo',
  function ($rootScope, $base64, $http, $hengtexTopAlert, UserInfo) {
      return {
          post: httpPost
      };
   ...
   //本地存储数据
  .factory('$hengtexLocals', ['$window', function ($window) {
      return {
          //存储单个属性
  ...
  //弹层
  .factory('$hengtexPopup', ['$rootScope', '$ionicPopup', function ($rootScope,$ionicPopup) {
      return {
          confirm: confirm
      }
  ...
  //触发下拉刷新
  .service('$hengtexTriggerRefresh', ['$timeout', '$ionicScrollDelegate', function ($timeout, $ionicScrollDelegate) {
      /**
  ...
  //格式化时间
  .factory('$hengtexFormatDate', function () {
  ...
  //页面模型
  .factory('$hengtexPageModal',['$ionicModal',function($ionicModal){
  ...
  //选择页面模型
  .factory('$hengtexSelectModal',['$rootScope','$ionicModal', function ($rootScope,$ionicModal) {
  ...
  //判断数据是否完整
  .factory('$hengtexDataIsAll',function () {
    return {
   ...
   //获取数据字典的数据
   .factory('$hengtexGetDataDict', ['$hengtexHttp', 'ApiUrl', function ($hengtexHttp, ApiUrl) {
     return function (ops) {
       $hengtexHttp.post({
         "url": ApiUrl.getDataDict,
         "data": { "DataType": ops.itemName },
         "isverify": true,
         "success": function (data) {
           ...
           for (var i in data.result) {
             ...
             lsdata[i] = item;
             lsdata[i].itemName = item.nativeName;
             lsdata[i].itemValue = item.EenglishName;
             //lsdata.push(item);
  ...
  //获取数据字典的数据
  .factory('$hengtexGetDataItem',['$hengtexHttp','ApiUrl',function($hengtexHttp,ApiUrl){
    return function (ops) {
      $hengtexHttp.post({
  ...
  //工作流表单转换
  .factory('$hengtexFormDesign', function () {
        return {
...
//动态加载js
.factory('$hengtexLoadJs', ['$http', '$q', function ($http, $q) {
    return function (url) {
