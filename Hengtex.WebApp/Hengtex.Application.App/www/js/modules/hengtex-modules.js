...
* */
//主页应用列表
 app .factory('HomeApps',['$hengtexHttp', 'ApiUrl', '$hengtexTopAlert','UserInfo', function ($hengtexHttp, ApiUrl, $hengtexTopAlert,UserInfo) {
      var homeApps = [];
      ...
      return {
          init: function () {
              ...
              $hengtexHttp.post({
                  ...
                  "data": { "Account": UserInfo.Account, "DomainName": "homeApps" },
                  "success": function (data) {
                      //homeApps = [];
                      homeApps.splice(0, homeApps.length);
                      for (var i in  data.result) {
                          var item = data.result[i];
                          var obj = {};
                          obj.name = data.result[i].fullName;
                          obj.icon = data.result[i].icon;
                          obj.color = data.result[i].color;
                  ...
                  "error": function () {
                      $hengtexTopAlert.show({ text: "权限取得失败！请联系管理员！" });
                  }
              });
          },
        all: function () {
        ...
        removeAll: function () {
            homeApps.splice(0, homeApps.length);// homeApps=[];
        },
...
//实例列表信息
  .factory('Cases', ['$hengtexHttp', 'ApiUrl', '$hengtexTopAlert','UserInfo',function ($hengtexHttp, ApiUrl, $hengtexTopAlert, UserInfo) {
      var cases = [];
    //var cases = [{
    //  id: 0,
    //  name: '列表',
    ...
    return {
        init: function () {
            ...
            $hengtexHttp.post({
                "url": ApiUrl.getUserModuleListApi,
                "data": { "Account": UserInfo.Account, "DomainName": "AppsCase" },
                "success": function (data) {
                    cases.splice(0, cases.length);// cases=[];
                    for (var i in data.result) {
                        var item = data.result[i];
                        var obj = {};
...
//客户列表信息
  .factory('lrmCustomers',['$hengtexFormatDate', 'AreaInfo','$hengtexGetDataItem','$hengtexHttp','ApiUrl','$hengtexTopAlert',
    function ($hengtexFormatDate, AreaInfo,$hengtexGetDataItem,$hengtexHttp,ApiUrl,$hengtexTopAlert) {
    //客户列表
    var custList = {
      records: 0,
      page: 1,
    ...
    function translateData(data, obj) {
      //console.log(data);
        for (var i in data) {
          ...
          data[i].lastDate = item.modifyDate == null ? $hengtexFormatDate(item.createDate, 'MM-dd') : $hengtexFormatDate(item.modifyDate, 'MM-dd');
...
//商机列表信息
  .factory('lrmBusinesss',['$hengtexFormatDate','$hengtexGetDataItem','$hengtexHttp','ApiUrl','$hengtexTopAlert',
...
//发起流程
.factory('lrmFlowDesigns', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
  //发起流程列表数据
  var flowdesignList = {
    records: 0,
    page: 1,
  ...
  function translateData(data, obj) {
    for (var i in data) {
      var item = data[i];
      data[i].f_createdate = $hengtexFormatDate(item.f_createdate, 'MM-dd');
      obj.push(data[i]);
    }
  };
  //获取数据方法
...
//草稿流程
.factory('lrFlowRoughdrafts', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
    //草稿流程列表数据
...
//我的流程列表
.factory('lrWorkflows', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
    //我的流程列表数据
    var workflowList = {
        records: 0,
...
//代办流程
.factory('lrFlowBefProcesss', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
    //代办流程列表数据
...
//已办流程
.factory('lrFlowAftProcesss', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
    //已办流程列表数据
...
//工作委托
.factory('lrFlowDelegates', ['$hengtexFormatDate', '$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', '$hengtexGetDataItem',
  function ($hengtexFormatDate, $hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexGetDataItem) {
  //发起流程列表数据
  ...
  function translateData(data, obj) {
      for (var i in data) {
          var item = data[i];
          data[i].f_createdate = $hengtexFormatDate(item.f_createdate, 'MM-dd');
          data[i].f_begindate = $hengtexFormatDate(item.f_begindate, 'MM-dd');
          data[i].f_enddate = $hengtexFormatDate(item.f_enddate, 'MM-dd');
          obj.push(data[i]);
      }
  };
  //获取数据方法
...
//即时通讯
    .factory('lrmIM', ['$q', 'UserInfo', 'SignalrUrl', '$hengtexLoadJs', '$hengtexFormatDate', function ($q, UserInfo, SignalrUrl, $hengtexLoadJs, $hengtexFormatDate) {
        var imServer = {
        ...
        function LastUserListToArray()
        {
            ...
            for (var i in imServer.lastUserList) {
                imServer.lastUserList[i].IMCreateTime = $hengtexFormatDate(imServer.lastUserList[i].IMCreateTime, "MM-dd hh:mm");
        ...
        return {
            "init": function (fnDisconnected,$scope) {
                ...
                if (!imServer.isIMLoaded) {
                    $hengtexLoadJs(SignalrUrl+"/hubs").then(
                        function (data) {
                            if (data == "success") {
  ...
  //首页公告
  .factory('Announces', ['$hengtexHttp', 'ApiUrl', '$hengtexTopAlert', function ($hengtexHttp, ApiUrl, $hengtexTopAlert) {
      var announces = [];
    //var announces = [{
    //    id: 0,
    //    name: '新一代快速开发框架是什么',
    ...
    return {
        init: function () {
            $hengtexHttp.post({
                "url": ApiUrl.getAnnouncesApi,
                "data": { "Account": "" },
                "success": function (data) {
                    //homeApps = [];
                    announces.splice(0, announces.length);
                    for (var i in data.result) {
                        var item = data.result[i];
                        //var obj = {};
                        //obj.name = data.result[i].fullName;
  ...
  .factory('lrmOrderInfos',[
    '$hengtexHttp','ApiUrl','$hengtexTopAlert','$hengtexFormatDate',
    function ($hengtexHttp, ApiUrl,$hengtexTopAlert,$hengtexFormatDate) {
      //客户列表
      var orderInfoList = {
        records: 0,
        page: 1,
      ...
      function translateData(data, obj) {
        for (var i in data) {
          ...
          data[i].createDate = $hengtexFormatDate(item.createDate, 'yyyy-MM-dd');
          data[i].modifyDate = $hengtexFormatDate(item.modifyDate, 'yyyy-MM-dd');
          data[i].paymentDate = $hengtexFormatDate(item.paymentDate, 'yyyy-MM-dd');
          data[i].orderDate = $hengtexFormatDate(item.orderDate, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
      };
      //获取数据
    ...
    function ($hengtexHttp, ApiUrl, $hengtexTopAlert, $hengtexFormatDate) {
      ...
      function translateData(data, obj) {
        for (var i in data) {
          ...
          data[i].f_CreateDate = $hengtexFormatDate(item.f_CreateDate, 'yyyy-MM-dd');
          data[i].f_ModifyDate = $hengtexFormatDate(item.f_ModifyDate, 'yyyy-MM-dd');
          data[i].f_OrderDate = $hengtexFormatDate(item.f_OrderDate, 'yyyy-MM-dd');
          data[i].f_PaymentDate = $hengtexFormatDate(item.f_PaymentDate, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
      };
      //获取数据
  ...
  .factory('lrmReceivableReports',[
    '$hengtexHttp','$hengtexFormatDate','$hengtexTopAlert','ApiUrl',
    function ($hengtexHttp, $hengtexFormatDate, $hengtexTopAlert, ApiUrl) {
      //客户列表
      ...
      function translateData(data, obj) {
        for (var i in data) {
          ...
          data[i].f_CreateDate = $hengtexFormatDate(item.f_CreateDate, 'yyyy-MM-dd');
          data[i].f_PaymentTime = $hengtexFormatDate(item.f_PaymentTime, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
      };
      //获取数据
  ...
  //我的账号信息
  .factory('UserInfo',['$hengtexLocals',function ($hengtexLocals) {
    var userInfo = {
        userid: "",
        account: "",
        password: "",
    ...
    return {
        get: function () {
            var newUserInfo = $hengtexLocals.getObject('userInfo');
        ...
        set: function (data, token, isLogin) {
            ...
            $hengtexLocals.setObject('userInfo', userInfo);
        },
        clear: function ()
        {
            userInfo.isLogin = false;
            userInfo.token = "";
            $hengtexLocals.setObject('userInfo', {});
        }
    };
}])
  //行政区域数据
  .factory('AreaInfo', ['$hengtexHttp','ApiUrl',function ($hengtexHttp, ApiUrl) {
    var areaInfo = {};
    return {
      init: function () {
        $hengtexHttp.post({
  ...
  //基础信息
  .factory('lrmBaseInfo',['$hengtexHttp','ApiUrl', function ($hengtexHttp,ApiUrl) {
    var baseInfo ={
      userListInfo:[]//用户列表
    };
    return{
