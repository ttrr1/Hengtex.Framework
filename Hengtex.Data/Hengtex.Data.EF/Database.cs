using Hengtex.Data.EF.Extension;
using Hengtex.Util;
    ...
    public class Database : IDatabase
    {
        ...
        public IEnumerable<T> FindList<T>(Expression<Func<T, bool>> condition, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class,new()
        {
            ...
            foreach (string item in _order)
            {
                ...
                var parameter = Expression.Parameter(typeof(T), "t");
                var property = typeof(T).GetProperty(_orderField);
                //var propertyAccess = property == null ? (Expression)Expression.Equal(Expression.Constant("1"), Expression.Constant("1")) : (Expression)Expression.MakeMemberAccess(parameter, property);
                //if (property != null)
                //{
